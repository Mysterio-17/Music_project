...
const gameData = {};
let currentRounds = {};
let leaderboard = {}; // Moved outside for global persistence
...
module.exports = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on("submitAnswer", ({ roomCode, username, answer }) => {
      ...
      round.responses[username] = {
        answer,
        time: responseTime,
      };
    });

    socket.on("readyForGame", ({ roomCode, username }) => {
      socket.join(roomCode);
    });
  });
  ...
  async function startNextRound(roomCode, io, songs) {
    ...
    const questionData = await generateQuestion(song.title);
  ...
  function updateLeaderboard(roomCode, roundScores) {
    ...
    roundScores.forEach(({ username, score }) => {
      leaderboard[roomCode][username] =
        (leaderboard[roomCode][username] || 0) + score;
    });
